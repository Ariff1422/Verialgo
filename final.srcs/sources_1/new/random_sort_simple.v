...
// SIMPLIFIED RANDOM SORT MODULE (Merge Sort)
module random_sort_simple #(
    ...
    // Merge boundary outputs for visualization
    output wire [ADDR_WIDTH:0] merge_left_start,
    output wire [ADDR_WIDTH:0] merge_left_end,
    output wire [ADDR_WIDTH:0] merge_right_start,
    output wire [ADDR_WIDTH:0] merge_right_end,
    output wire merge_in_progress
);
    ...
    wire sort_done;
    wire sort_busy;
    wire [ADDR_WIDTH-1:0] merge_write_idx_internal;
    ...
    assign capture_write_idx = merge_write_idx_internal;
    ...
    wire capture_src_is_live_internal;
    assign src_is_live = capture_src_is_live_internal;
    
    wire [ADDR_WIDTH:0] capture_merge_size_internal;
    wire [ADDR_WIDTH:0] capture_left_start_internal;
    assign merge_left_start = capture_left_start_internal;
    assign merge_left_end = capture_left_start_internal + capture_merge_size_internal;
    assign merge_right_start = capture_left_start_internal + capture_merge_size_internal;
    assign merge_right_end = (capture_left_start_internal + 2*capture_merge_size_internal > active_array_length) ? 
                             active_array_length : 
                             (capture_left_start_internal + 2*capture_merge_size_internal);
    assign merge_in_progress = (state == SORTING) && comparison_active;
    ...
    merge_sort_engine #(.DATA_WIDTH(DATA_WIDTH), .MAX_ARRAY_SIZE(MAX_ARRAY_SIZE), .ADDR_WIDTH(ADDR_WIDTH)) 
    sort_engine (
        .clk(clk), .rst(rst),
        ...
        .debug_pass_state(), .debug_merge_state(),
        .operation_type(operation_type),
        .capture_valid(capture_valid),
        .capture_write_idx(merge_write_idx_internal),
        .capture_merge_size(capture_merge_size_internal),
        .capture_left_start(capture_left_start_internal),
        .capture_left_idx(), .capture_right_idx(),
        .capture_left_val(), .capture_right_val(),
        .capture_src_is_live(capture_src_is_live_internal),
